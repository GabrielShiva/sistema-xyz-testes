...
const uint16_t MAX_INTERVAL_JOYSTICK = 500; // us
...
// Find index of saved position by character
int find_saved_position_index(char character) {
    for (int i = 0; i < MAX_SAVED_POSITIONS; i++) {
...
// Parse SAVEPOS command: SAVEPOS,<character>,<x_pos>,<y_pos>
void parse_savepos_command(const char* params) {
    char* param_copy = malloc(strlen(params) + 1);
    strcpy(param_copy, params);
    ...
    // Validate character (must be alphanumeric)
    ...
    // Find existing position or empty slot
...
// Parse RECALLPOS command: RECALLPOS,<character>
void parse_recallpos_command(const char* params) {
    if (strlen(params) != 1) {
    ...
    // Stop all motors before movement
    stop_all_motors();
    sleep_ms(10);
    ...
    // Move motor 1 (Y-axis) if available
    if (active_motor_count >= 2) {
...
// Parse CLEARPOS command
void parse_clearpos_command(void) {
    init_saved_positions(); // Reset all positions
